...
import { moduleShortCode } from './constants/modules';
...
import { DialogProvider, showPrompt } from './components/ui/Dialogs';
...
import type { AppState, ModuleKey, PidioConfig, Project } from './types/app.types';
...
type PerformanceState = {
  performance?: { mode?: string; refreshMs?: number };
  metrics?: { cpu?: number; memory?: number; activeRenders?: number };
  encoder?: string;
};
...
export function App() {
  ...
  const [config, setConfig] = useState<PidioConfig | null>(null);
  const [saveInfo, setSaveInfo] = useState('');
  const [perf, setPerf] = useState<PerformanceState | null>(null);
  const saveTimer = useRef<number | null>(null);
  const pendingConfig = useRef<PidioConfig | null>(null);
  const undoRedo = useUndoRedo<PidioConfig | null>(null);
  ...
  useEffect(() => {
    ...
    const t = setInterval(
      ...
      Number(perf?.performance?.refreshMs || 1500),
    );
    return () => {
      window.clearTimeout(kickoff);
      clearInterval(t);
    };
  }, [perf?.performance?.refreshMs, refresh, refreshPerf]);
  useEffect(
    () => () => {
  ...
  const updateConfig = useCallback(
    ...
    [config, undoRedo, scheduleConfigSave],
  );
  ...
  const savePreset = useCallback(async () => {
    const name = (await showPrompt('Nama preset?')) || 'Preset Baru';
  ...
  const statusText = useMemo(
    () =>
      activeJob
        ? `${activeJob.title} / ${activeJob.progress || 0}%`
        : systemReady
          ? saveInfo || 'Siap produksi'
  ...
  return (
    <div
      className={cn(
        'h-screen flex flex-col bg-gradient-to-br from-ds-bg-base via-ds-bg-base to-ds-bg-elevated text-ds-text overflow-hidden',
        systemReady ? 'appReady' : 'appLoading',
      )}
    >
      ...
      <header className="h-10 shrink-0 flex items-center gap-4 px-4 bg-gradient-to-r from-ds-bg-elevated via-ds-bg-panel to-ds-bg-elevated border-b-2 border-ds-line-strong shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_4px_12px_rgba(0,0,0,0.4)] z-10 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-bold tracking-[0.08em] text-ds-title drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]">
            PidioForge
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 text-[11px] text-ds-muted">
          <span
            className={cn(
              ...
              activeJob
                ? 'bg-ds-danger border-ds-danger shadow-[0_0_12px_rgba(248,113,113,0.8),inset_0_1px_0_rgba(255,255,255,0.3)] animate-pulse'
                : systemReady
                  ? 'bg-ds-success border-ds-success shadow-[0_0_12px_rgba(52,211,153,0.8),inset_0_1px_0_rgba(255,255,255,0.3)]'
                  : 'bg-ds-muted border-ds-muted shadow-[0_0_8px_rgba(142,162,184,0.5)] animate-pulse',
            )}
          />
          <span className="truncate max-w-[200px] font-medium drop-shadow-[0_1px_2px_rgba(0,0,0,0.3)]">
            {statusText}
          </span>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          {modules.map(item => {
            ...
            return (
              <button
                ...
                className={cn(
                  'relative flex items-center justify-center w-11 h-11 rounded-xl transition-all duration-150 ease-out border-2 will-change-transform overflow-hidden',
                  isActive
                    ? cn(
                        'bg-gradient-to-br from-ds-bg-panel-2 to-ds-bg-panel border-ds-line-strong shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_4px_8px_rgba(0,0,0,0.4)]',
                        toneAccent[tone] || 'text-ds-accent',
                      )
                    : 'border-ds-line hover:border-ds-line-strong hover:shadow-[0_2px_6px_rgba(0,0,0,0.3)]',
                )}
              >
                <span
                  className={cn(
                ...
                {isActive && (
                  <span
                    className={cn(
                      'absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 rounded-r-full shadow-[0_0_10px_currentColor]',
                      `bg-current`,
                    )}
                  />
                )}
                <span className="relative z-10 flex flex-col items-center justify-center gap-0.5">
                  <i
                    className={cn(
                      'block w-6 h-6',
                      isActive ? 'drop-shadow-[0_0_10px_currentColor]' : toneAccent[tone] || 'text-ds-accent',
                    )}
                  >
                  ...
                  <small
                    className={cn(
                      'text-[7px] leading-none font-bold tracking-[0.04em]',
                      isActive ? 'text-white' : 'text-ds-text',
                    )}
                  >
                    {moduleShortCode[item.key]}
                  </small>
                </span>
              </button>
            );
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          <PanelErrorBoundary fallbackTitle="Preview">
            <PreviewPane
              active={active}
              jobs={state?.jobs || []}
              logs={state?.logs || []}
        ...
        <div className="flex items-center gap-4 ml-auto">
          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded border-2 border-ds-line bg-ds-bg-panel shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_2px_4px_rgba(0,0,0,0.2)]">
            ...
            <span
              className={cn(
                'font-bold text-[11px]',
                (perf?.metrics?.cpu ?? 0) > 80
                  ? 'text-ds-warn drop-shadow-[0_0_4px_rgba(251,191,36,0.6)]'
                  : 'text-ds-text',
              )}
            >
              {perf?.metrics?.cpu ?? 0}%
          ...
          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded border-2 border-ds-line bg-ds-bg-panel shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_2px_4px_rgba(0,0,0,0.2)]">
            ...
            <span
              className={cn(
                'font-bold text-[11px]',
                (perf?.metrics?.memory ?? 0) > 80
                  ? 'text-ds-warn drop-shadow-[0_0_4px_rgba(251,191,36,0.6)]'
                  : 'text-ds-text',
              )}
            >
              {perf?.metrics?.memory ?? 0}%
            </span>
          </div>
          ...
          {renderCount > 0 && (
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-ds-danger-bg border-2 border-ds-danger shadow-[inset_0_1px_0_rgba(255,255,255,0.1),0_4px_8px_rgba(248,113,113,0.5),0_2px_4px_rgba(0,0,0,0.3)]">
              ...
              <span className="font-bold text-ds-danger text-[11px] drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
                Render {renderCount}
              </span>
            </div>
          )}
