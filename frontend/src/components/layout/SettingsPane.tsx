  ...
  return (
    <aside
      className={cn(
        ...
        collapsed ? 'w-0 opacity-0' : 'w-[340px] min-w-[340px]'
      )}
    >
      ...
      <div className="flex items-center justify-between h-11 px-4 border-b-2 border-ds-line-strong shrink-0 bg-gradient-to-r from-ds-bg-panel to-ds-bg-panel-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_2px_4px_rgba(0,0,0,0.3)]">
        <span className="text-[12px] font-bold text-ds-title tracking-wider uppercase truncate">
          {title}
        </span>
        <button
          onClick={onToggle}
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin px-1">
        {children}
      </div>
    </aside>
  )
