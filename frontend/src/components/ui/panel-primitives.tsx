...
export function Grid3({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn('grid grid-cols-3 gap-2 my-1 min-w-0', className)}>
      {children}
    </div>
  )
}
...
export function ActionBar({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn('flex flex-wrap items-center gap-1.5 px-4 py-2 bg-ds-bg-panel border-b border-ds-line', className)}>
      {children}
    </div>
  )
}
...
export function ActionBtn({
  ...
  return (
    <button
      ...
      className={cn(
        'inline-flex items-center justify-center gap-1.5 rounded-md text-[11px] font-medium border',
        'h-7 px-3 disabled:opacity-40 disabled:cursor-not-allowed transition-colors',
        variant === 'primary' && 'bg-ds-accent text-white border-ds-accent hover:brightness-90',
        variant === 'danger' && 'bg-ds-danger text-white border-ds-danger hover:brightness-90',
        variant === 'wide' && 'w-full bg-ds-bg-panel-2 text-ds-text border-ds-line hover:bg-ds-bg-hover hover:border-ds-line-strong',
        variant === 'ghost' && 'bg-transparent text-ds-muted border-transparent hover:bg-ds-bg-hover hover:text-ds-text',
        variant === 'default' && 'bg-ds-bg-panel-2 text-ds-text border-ds-line hover:bg-ds-bg-hover hover:border-ds-line-strong',
      )}
    >
  ...
  return (
    <div className="mx-3 my-2 rounded-md bg-ds-warn-bg border border-ds-warn/25 px-3 py-2">
      ...
      <div className="flex flex-col gap-1">
        {items.map((w, i) => (
          <small key={i} className="text-[11px] text-ds-muted leading-relaxed">{w}</small>
        ))}
      </div>
    </div>
  )
...
export function Hint({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[10px] text-ds-subtle italic leading-relaxed px-2 my-1">{children}</p>
  )
}
  ...
  return (
    <div className="grid grid-cols-3 gap-2 px-3 py-2">
      {items.map(({ label, value }) => (
        <div key={label} className="flex flex-col items-center justify-center text-center bg-ds-bg-panel border border-ds-line rounded-md p-2 min-h-[52px]">
