...
export function ProjectTabs({ projects, activeProjectId, onSwitch, onRefresh }: ProjectTabsProps) {
  ...
  async function renameProject(id: string) {
    ...
    const name = prompt('Nama baru:', project?.name || '');
    if (!name) return;
    try {
  ...
  async function deleteProject(id: string) {
    if (projects.length <= 1) {
      alert('Tidak bisa menghapus project terakhir.');
      return;
    }
  ...
  return (
    <div className="flex items-center gap-0.5 px-2 py-1 bg-[#080d14] border-b border-[rgba(142,162,184,0.12)] overflow-x-auto" onClick={() => setContextMenu(null)}>
      {projects.map(p => (
        <button
          key={p.id}
      ...
      {contextMenu && (
        ...
        >
          <button onClick={() => renameProject(contextMenu.id)} className="block w-full text-left px-3 py-1.5 text-[10px] text-[#dce8ef] hover:bg-[#1a2030]">Rename</button>
          <button onClick={() => duplicateProject(contextMenu.id)} className="block w-full text-left px-3 py-1.5 text-[10px] text-[#dce8ef] hover:bg-[#1a2030]">Duplikat</button>
          <button onClick={() => deleteProject(contextMenu.id)} className="block w-full text-left px-3 py-1.5 text-[10px] text-[#e76d78] hover:bg-[#1a2030]">Hapus</button>
        </div>
      )}
    </div>
  );
