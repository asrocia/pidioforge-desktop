...
export function BrandingPanel({ config, updateConfig }: { config: any; updateConfig: (path: string, value: any) => void }) {
  ...
  return (<div className="space-y-4">
        ...
        <div className="p-4 bg-[var(--secondary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-lg)] space-y-3">
          ...
          <Callout type="tip">
            Edit posisi branding langsung di layar PREVIEW kanan. Geser logo, CTA, atau watermark; klik watermark untuk ubah teks.
          </Callout>
          {message && (
        ...
        >
          ...
          <Field label="Transisi">
            <SelectInput value={getDeep(config, 'branding.bumperTransition', 'fade')} onChange={v => updateConfig('branding.bumperTransition', v)}>
          ...
          <Field label="Durasi Intro"><TextInput type="number" value={getDeep(config, 'branding.bumperDurationIntro', getDeep(config, 'branding.bumperDuration', 3))} onChange={v => updateConfig('branding.bumperDurationIntro', v)} /></Field>
          
          <Field label="Durasi Outro"><TextInput type="number" value={getDeep(config, 'branding.bumperDurationOutro', getDeep(config, 'branding.bumperDuration', 3))} onChange={v => updateConfig('branding.bumperDurationOutro', v)} /></Field>
          ...
          <Field label="Audio Utama">
            <SelectInput value={getDeep(config, 'branding.bumperAudioMode', 'after-intro')} onChange={v => updateConfig('branding.bumperAudioMode', v)}>
          ...
          <Callout type="tip">
            Intro & outro dapat memiliki durasi berbeda. Audio ducking akan menurunkan volume audio utama saat bumper diputar.
          </Callout>
        </Card>
        ...
        <Card title="Logo Overlay">
          ...
          <SliderControl label="Scale Logo" value={Number(getDeep(config, 'branding.logoScale', 18))} onChange={v => updateConfig('branding.logoScale', v)} min={5} max={100} />
          <SliderControl label="Opacity Logo" value={Number(getDeep(config, 'branding.logoOpacity', 100))} onChange={v => updateConfig('branding.logoOpacity', v)} min={0} max={100} />
        ...
        <Card title="CTA Greenscreen">
          ...
          <SliderControl label="Scale CTA" value={Number(getDeep(config, 'branding.ctaScale', 26))} onChange={v => updateConfig('branding.ctaScale', v)} min={5} max={100} />
          <div className="space-y-3">
        ...
        <Card title="Animated Text Overlays">
          ...
          <SliderControl label="Background Opacity" value={Number(getDeep(config, 'branding.textOverlay.bgOpacity', 80))} onChange={v => updateConfig('branding.textOverlay.bgOpacity', v)} min={0} max={100} />
        </Card>
