...
export function LyricsPanel({ config, updateConfig }: { config: any; updateConfig: (path: string, value: any) => void }) {
  ...
  return (<div className="space-y-4">
        ...
        <Card title="Smart Sync & Waveform">
          ...
          <SliderControl label="Waveform Height" value={Number(getDeep(config, 'lyrics.smartSync.waveformHeight', 80))} onChange={v => updateConfig('lyrics.smartSync.waveformHeight', v)} min={40} max={200} />
        ...
        <Card title="Timing & Output">
          ...
          <Check label="Smart timing berbobot" checked={Boolean(getDeep(config, 'lyrics.smartTiming', true))} onChange={v => updateConfig('lyrics.smartTiming', v)} />
          <Check label="Beat snap" checked={Boolean(getDeep(config, 'lyrics.beatSnap', true))} onChange={v => updateConfig('lyrics.beatSnap', v)} />
        ...
        <Card title="Style Lirik">
          ...
          <SliderControl label="Skala" value={Number(getDeep(config, 'lyrics.scale', 28))} onChange={v => updateConfig('lyrics.scale', v)} min={16} max={60} />
          <SliderControl label="Outline" value={Number(getDeep(config, 'lyrics.outline', 2))} onChange={v => updateConfig('lyrics.outline', v)} min={0} max={6} />
          <SliderControl label="Shadow" value={Number(getDeep(config, 'lyrics.shadow', 1))} onChange={v => updateConfig('lyrics.shadow', v)} min={0} max={5} />
          <Check label="Karaoke" checked={Boolean(getDeep(config, 'lyrics.karaoke', false))} onChange={v => updateConfig('lyrics.karaoke', v)} />
          <Check label="Word-by-word" checked={Boolean(getDeep(config, 'lyrics.wordByWord', false))} onChange={v => updateConfig('lyrics.wordByWord', v)} />
          <Check label="Uppercase" checked={Boolean(getDeep(config, 'lyrics.uppercase', false))} onChange={v => updateConfig('lyrics.uppercase', v)} />
