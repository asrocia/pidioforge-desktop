...
export function HelpPanel() {
  const guides = [
    [
      'Sumber & Engine',
      'Pilih visual, audio, judul, output folder. Jalankan Scan & Cek sebelum membuat batch atau render.',
    ],
    ...
    [
      'Spektrum & Now Playing',
      'Atur bar/wave/line, warna, posisi, progress bar, dan teks now playing. Matikan Spectrum, Progress Bar, dan Now Playing untuk output bersih.',
    ],
    ...
    [
      'Looping',
      'Pilih video pendek, Auto Detect Loop, Preview Sambungan, pilih crossfade/ping-pong/trim, lalu Buat Looping.',
    ],
    [
      'Preview Kanan',
      'Visual langsung play. Drag layer logo, spectrum, timestamp, dan lower third. Render preview atau kirim ke antrian.',
    ],
    ...
    <div className="space-y-4">
      ...
      <Card title="Alur Cepat">
        <div className="space-y-2">
          {quickFlow.map((step, i) => (
            <div
              key={step}
              className="flex items-start gap-3 p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)]"
            >
      ...
      <Card title="Panduan Fitur">
        <div className="grid grid-cols-2 gap-3">
          {guides.map(([title, text]) => (
            <div
              key={title}
              className="p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)] hover:border-[var(--accent-primary)]/30 transition-all duration-200"
            >
      ...
      <Card title="Tips Penting">
        <div className="p-3 bg-[var(--accent-warning)]/10 border-l-4 border-[var(--accent-warning)] rounded-[var(--radius-lg)] space-y-2">
          <div className="flex items-start gap-2">
            ...
            <small className="text-[11px] text-[var(--text-primary)] leading-relaxed">
              Restart aplikasi setelah update backend atau build baru.
            </small>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-[var(--accent-warning)] text-[14px]">⚠</span>
            <small className="text-[11px] text-[var(--text-primary)] leading-relaxed">
              Untuk loop panjang, gunakan Validasi Output dan tombol Batal bila proses terlalu lama.
            </small>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-[var(--accent-warning)] text-[14px]">⚠</span>
            <small className="text-[11px] text-[var(--text-primary)] leading-relaxed">
              Untuk hasil tanpa spectrum, matikan Spectrum, Progress Bar, dan Now Playing.
            </small>
          </div>
        </div>
      </Card>
      ...
      <Card title="Keyboard Shortcuts">
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div
              key={key}
              className="flex items-center justify-between p-2 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)]"
            >
              <span className="text-[11px] text-[var(--text-muted)]">{action}</span>
              <kbd className="px-2 py-1 text-[10px] font-mono font-bold text-[var(--text-primary)] bg-[var(--primary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-sm)]">
                {key}
              </kbd>
            </div>
          ))}
        ...
        <div className="space-y-3">
          <div className="p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)]">
            ...
            <p className="text-[10px] text-[var(--text-muted)] leading-relaxed mb-2">
              Install FFmpeg and add to system PATH. Restart application after installation.
            </p>
            <a
          ...
          <div className="p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)]">
            ...
            <p className="text-[10px] text-[var(--text-muted)] leading-relaxed">
              Check: 1) File paths are valid, 2) Sufficient disk space, 3) No special characters in filenames, 4) FFmpeg
              is working.
            </p>
          </div>
          <div className="p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)]">
            ...
            <p className="text-[10px] text-[var(--text-muted)] leading-relaxed">
              Try: 1) Refresh browser, 2) Clear cache, 3) Check if backend server is running, 4) Verify file formats are
              supported.
            </p>
          </div>
          <div className="p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)]">
            ...
            <p className="text-[10px] text-[var(--text-muted)] leading-relaxed">
              Solutions: 1) Use GPU acceleration, 2) Lower resolution/quality, 3) Reduce concurrent jobs, 4) Close other
              applications.
            </p>
          </div>
        </div>
      ...
      <Card title="Frequently Asked Questions">
        <div className="space-y-3">
          {[
            ...
            <div
              key={q}
              className="p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)]"
            >
      ...
      <Card title="Video Tutorials">
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div
              key={title}
              className="p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)] hover:border-[var(--accent-primary)]/30 transition-all duration-200 cursor-pointer"
            >
              <div className="flex items-center justify-between mb-2">
                <b className="text-[11px] text-[var(--text-primary)]">{title}</b>
                <span className="text-[9px] px-2 py-0.5 rounded-full bg-[var(--accent-primary)]/20 text-[var(--accent-primary)]">
                  {level}
                </span>
              </div>
      ...
      <Card title="Support & Community">
        <div className="grid grid-cols-3 gap-3">
          <a
            href="#"
            className="p-3 bg-[var(--tertiary-bg)] border border-[var(--border-subtle)] rounded-[var(--radius-md)] hover:border-[var(--accent-primary)]/30 transition-all duration-200 text-center"
          >
