...
export function runBuffer(command, args = []) {
  return new Promise(resolve => {
    ...
    child.on('close', code =>
      resolve({ ok: code === 0, code, stdout: Buffer.concat(stdout), stderr: Buffer.concat(stderr).toString() }),
    );
  });
}
...
export function runLoopFfmpeg(args = [], duration = 0, callbacks = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', code =>
      code === 0 ? resolve({ ok: true }) : reject(new Error(stderr || `FFmpeg loop gagal kode ${code}`)),
    );
  });
}
...
export async function ffmpegDiagnostics() {
  ...
  const encoders = {
    libx264: /libx264/.test(text),
    h264_nvenc: /h264_nvenc/.test(text),
    h264_qsv: /h264_qsv/.test(text),
    h264_amf: /h264_amf/.test(text),
  };
  return {
    ffmpeg: version.ok,
    ffprobe: (await runCmd(FFPROBE, ['-version'])).ok,
    ...binaryDiagnostics(),
    version: version.stdout.split('\n')[0] || version.stderr.split('\n')[0] || '',
    encoders,
    recommended: encoders.h264_nvenc ? 'nvidia' : encoders.h264_qsv ? 'intel' : encoders.h264_amf ? 'amd' : 'cpu',
  };
}
...
export async function diskStats(dir = process.cwd()) {
  try {
    ...
    const r = await runCmd(
      ...
      process.platform === 'win32' ? ['logicaldisk', 'get', 'size,freespace,caption'] : ['-k', root],
    );
    ...
    if (process.platform === 'win32') {
      ...
      const free = Number(first[1] || 0),
        total = Number(first[2] || 0);
      return {
        ok: true,
        usedPercent: total ? Math.round((1 - free / total) * 100) : 0,
    ...
    return { ok: false, usedPercent: 0, freeGB: 0, totalGB: 0 };
  }
}
