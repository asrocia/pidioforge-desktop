...
export function isImage(file) {
  return imageExts.has(ext(file));
}
export function isVideo(file) {
  return videoExts.has(ext(file));
}
export function isAudio(file) {
  return audioExts.has(ext(file));
}
export function escapeFilter(file) {
...
export function buildAssDocument({
  ...
  timestampText = 'Rendered by PidioForge',
  ...
  watermarkInterval = 12,
  watermarkVisibleDuration = 5,
  include = 'all',
  playResX = 1280,
  playResY = 720,
}) {
  ...
  const events = [];
  const styles = [];
  const alpha = Math.max(0, Math.min(255, Math.round(255 - (Number(watermarkOpacity ?? 70) / 100) * 255)))
    .toString(16)
    .padStart(2, '0')
    .toUpperCase();
  const wmAlign = watermarkPosition.includes('Kanan')
    ? watermarkPosition.includes('Atas')
      ? 9
      : 3
    : watermarkPosition.includes('Atas')
      ? 7
      : watermarkPosition.includes('Tengah')
        ? 5
        : 1;
  const infoAlign = timestampPosition.includes('Kanan')
    ? timestampPosition.includes('Bawah')
      ? 3
      : 9
    : timestampPosition.includes('Bawah')
      ? 1
  ...
  const shouldInclude = layer => include === 'all' || include === layer;

  if (shouldInclude('nowPlaying')) {
    styles.push(
  ...
  if (shouldInclude('timestamp')) {
    styles.push(
      `Style: Info,${font},18,&H00FFFFFF,&H000000FF,&HAA000000,&H66000000,0,0,0,0,100,100,0,0,1,1,1,${infoAlign},20,20,20,1`,
    );
    if (timestampEnabled)
      events.push(
        `Dialogue: 0,${assTime(0)},${assTime(safeDuration)},Info,,0,0,0,,${assEscape(timestampText || 'Rendered by PidioForge')}`,
      );
  }

  if (shouldInclude('lowerThird')) {
    styles.push(
    ...
    if (watermarkEnabled && watermark) {
      if (watermarkMode === 'interval') {
        ...
        for (let t = 0; t < safeDuration; t += interval) {
          events.push(
            `Dialogue: 0,${assTime(t)},${assTime(Math.min(safeDuration, t + visible))},Watermark,,0,0,0,,${assEscape(watermark)}`,
          );
        }
      } else {
  ...
  if (!styles.length && !events.length) return '';
