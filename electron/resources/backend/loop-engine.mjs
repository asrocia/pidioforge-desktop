...
export function outputPresetArgs(preset = 'source') {
  if (preset === 'youtube1080') return ['-vf', 'scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2'];
  ...
  if (preset === 'wallpaper4k') return ['-vf', 'scale=3840:2160:force_original_aspect_ratio=decrease,pad=3840:2160:(ow-iw)/2:(oh-ih)/2'];
  return [];
}

export async function makeLoopUnit({ source, unitPath, trimStart, segmentDuration, loopStyle, crossfade, muteAudio, audioFade, inputInfo }) {
  ...
  if (loopStyle === 'pingpong') {
    ...
    if (!muteAudio && inputInfo.hasAudio) args.push('-map', '0:a?', '-af', audioFade ? `afade=t=in:st=0:d=${Math.min(1, segmentDuration / 4)},afade=t=out:st=${Math.max(0, segmentDuration - Math.min(1, segmentDuration / 4))}:d=${Math.min(1, segmentDuration / 4)}` : 'anull', '-c:a', 'aac', '-b:a', '192k');
    args.push('-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', unitPath);
  ...
  if (loopStyle === 'crossfade' && crossfade > 0.05) {
    ...
    const filters = [`[0:v][1:v]xfade=transition=fade:duration=${fade}:offset=${offset},trim=duration=${segmentDuration},setpts=PTS-STARTPTS,format=yuv420p[v]`];
    const args = ['-y', ...ss, ...t, '-i', source, ...ss, ...t, '-i', source, '-filter_complex', filters.join(';'), '-map', '[v]'];
    if (!muteAudio && inputInfo.hasAudio) args.push('-map', '0:a?', '-af', audioFade ? `afade=t=in:st=0:d=${fade},afade=t=out:st=${Math.max(0, segmentDuration - fade)}:d=${fade}` : 'anull', '-c:a', 'aac', '-b:a', '192k');
  ...
  else if (audioFade && inputInfo.hasAudio) args.push('-af', `afade=t=in:st=0:d=${Math.min(1, segmentDuration / 4)},afade=t=out:st=${Math.max(0, segmentDuration - Math.min(1, segmentDuration / 4))}:d=${Math.min(1, segmentDuration / 4)}`);
  args.push('-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '192k', '-movflags', '+faststart', unitPath);
  const result = await runCmd(FFMPEG, args);
  if (!result.ok) throw new Error(result.stderr || 'Loop unit gagal.');
}

export async function renderLoopVideo({ input = '', duration = 60, output = '', mode = 'copy', loopStyle = 'normal', crossfade = 0.5, trimStart = 0, trimEnd = 0, muteAudio = false, audioFade = false, preset = 'source' } = {}, callbacks = {}) {
  ...
  const segmentDuration = Math.max(0.2, end > start ? end - start : Math.max(0.2, sourceDuration - start || sourceDuration || targetDuration));
  const needsUnit = loopStyle !== 'normal' || start > 0 || end > 0 || muteAudio || audioFade || mode === 'encode' || preset !== 'source';
  const loopInput = needsUnit ? path.join(outDir, `${safeId('loop-unit')}.mp4`) : source;
  if (needsUnit) await makeLoopUnit({ source, unitPath: loopInput, trimStart: start, segmentDuration, loopStyle, crossfade: Number(crossfade || 0), muteAudio, audioFade, inputInfo });
  const args = ['-y', '-stream_loop', '-1', '-i', loopInput, '-t', String(targetDuration), '-map', '0:v:0', '-map', '0:a?'];
  const presetArgs = outputPresetArgs(preset);
  if (mode === 'copy' && !needsUnit && !presetArgs.length) args.push('-c', 'copy');
  else args.push(...presetArgs, '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '20', '-pix_fmt', 'yuv420p', muteAudio ? '-an' : '-c:a', muteAudio ? '' : 'aac', ...(muteAudio ? [] : ['-b:a', '192k']));
  ...
  return { ok: true, output: finalOutput, url: `/api/media/file?path=${encodeURIComponent(finalOutput)}`, inputDuration: Number(inputInfo.duration || 0), unitDuration: segmentDuration, duration: targetDuration, size: st?.size || 0, mode, loopStyle, preset };
}
...
export async function renderLoopSeamPreview(options = {}) {
  ...
  const args = ['-y', '-ss', String(tailStart), '-t', String(previewSeconds), '-i', source, '-ss', String(start), '-t', String(previewSeconds), '-i', source, '-filter_complex', filter, '-map', '[v]', '-an', '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '22', '-pix_fmt', 'yuv420p', output];
  const result = await runCmd(FFMPEG, args);
  ...
  return { ok: true, output, url: `/api/media/file?path=${encodeURIComponent(output)}`, size: st?.size || 0, previewSeconds, quality: quality?.best || null, candidates: quality?.candidates || [] };
}
...
export async function renderLoopBatch({ items = [], outputDir = '', ...options } = {}) {
  ...
  async function expandEntry(entry) {
    ...
    if (!st) return;
    if (st.isFile()) { await pushMedia(resolved); return; }
  ...
  return { ok: true, outputDir: outDir, created: results, skipped, scanned: expanded.length, limited: expanded.length >= limit };
}
...
export async function validateLoopOptions(options = {}) {
  ...
  return { ok: errors.length === 0, errors, warnings, inputDuration, loopsNeeded, disk, estimate: { duration: targetDuration, mode: options.mode || 'copy', preset: options.preset || 'source' }, error: errors.join(' ') };
}
...
export async function startLoopJob(options = {}, loopJobs) {
  ...
  const job = { id, status: 'running', progress: 0, etaSeconds: 0, elapsedSeconds: 0, renderedSeconds: 0, startedAt, options, validation };
  loopJobs.set(id, job);
  renderLoopVideo(options, {
    onProcess: child => { job.child = child; },
    onProgress: metrics => {
      ...
      job.etaSeconds = job.progress > 0 && job.progress < 100 ? Math.max(0, Math.round((job.elapsedSeconds / job.progress) * (100 - job.progress))) : 0;
      job.updatedAt = new Date().toISOString();
    },
  }).then(result => {
    Object.assign(job, result, { status: 'done', progress: 100, etaSeconds: 0, finishedAt: new Date().toISOString() });
  }).catch(error => {
    if (job.status === 'cancelled') return;
    Object.assign(job, { status: 'failed', error: error.message, finishedAt: new Date().toISOString() });
  }).finally(() => { delete job.child; });
...
export async function loopFrame(file, seconds) {
  ...
  const result = await runBuffer(FFMPEG, ['-v', 'error', '-ss', String(at), '-i', file, '-frames:v', '1', '-vf', 'scale=64:36:force_original_aspect_ratio=decrease,pad=64:36:(ow-iw)/2:(oh-ih)/2,format=gray', '-f', 'rawvideo', 'pipe:1']);
...
export async function analyzeLoopPoint({ input = '', trimStart = 0, trimEnd = 0, window = 1.5, samples = 8 } = {}) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const end = trimEnd > start ? Number(trimEnd) - searchWindow + ratio * searchWindow : duration - searchWindow + ratio * searchWindow;
    try {
      ...
      candidates.push({ trimStart: Number(start.toFixed(2)), trimEnd: Number(Math.max(start + 0.2, Math.min(duration, end)).toFixed(2)), score, label: loopQualityLabel(score) });
    } catch {}
  }
  candidates.sort((a, b) => b.score - a.score);
  const best = candidates[0] || { trimStart: Number(trimStart || 0), trimEnd: Number(trimEnd || duration), score: 0, label: 'Patah' };
  return { ok: true, duration, best, candidates: candidates.slice(0, 8), warnings: best.score < 72 ? ['Sambungan masih terlihat patah; coba crossfade atau ping-pong.'] : [] };
}
