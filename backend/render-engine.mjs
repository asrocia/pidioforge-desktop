...
function run(command, args) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', code =>
      code === 0 ? resolve(stdout.trim()) : reject(new Error(stderr.trim() || `${command} exit ${code}`)),
    );
  });
}
...
const KENBURNS_MODES = {
  'zoom-in': (w, h, fps, dur) =>
    `zoompan=z='min(zoom+0.001,1.15)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${fps * dur}:s=${w}x${h}:fps=${fps}`,
  'zoom-out': (w, h, fps, dur) =>
    `zoompan=z='if(eq(on,1),1.15,max(zoom-0.001,1.0))':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${fps * dur}:s=${w}x${h}:fps=${fps}`,
  'pan-left': (w, h, fps, dur) =>
    `zoompan=z='1.12':x='iw*0.12-iw*0.12*on/${fps * dur}':y='ih/2-(ih/zoom/2)':d=${fps * dur}:s=${w}x${h}:fps=${fps}`,
  'pan-right': (w, h, fps, dur) =>
    `zoompan=z='1.12':x='iw*0.0+iw*0.12*on/${fps * dur}':y='ih/2-(ih/zoom/2)':d=${fps * dur}:s=${w}x${h}:fps=${fps}`,
};
const KENBURNS_KEYS = Object.keys(KENBURNS_MODES);

export async function buildGallerySlideshowFile(config, workspaceDir, job) {
  const gallery = config.spectrum?.gallery;
  if (!gallery?.enabled || !Array.isArray(gallery.images)) return '';
  let media = gallery.images.filter(f => hasFile(f) && (isImage(f) || isVideo(f)));
  if (!media.length) return '';
  if (media.length === 1 && isImage(media[0]) && !gallery.kenBurns) return media[0];
  ...
  const out = path.join(workspaceDir, 'tmp', `${job.id || Date.now()}-gallery.mp4`);
  const { width, height } = targetSize(config);
  const fps = Number(config.target?.fps || 30);
  const perImage = Math.max(1, Number(gallery.duration || 5));
  const transitionName = GALLERY_TRANSITIONS[gallery.transition] || 'fade';
  const transitionDuration = Math.min(1.5, Math.max(0.3, perImage * 0.2));
  const kenBurns = Boolean(gallery.kenBurns);
  const kenBurnsMode = gallery.kenBurnsMode || 'random';

  const args = ['-y'];

  // Input: setiap item jadi segment video
  for (const item of media) {
    if (isImage(item)) {
      args.push('-loop', '1', '-framerate', String(fps), '-t', String(perImage), '-i', item);
    } else {
      // Video: trim to perImage duration
      args.push('-t', String(perImage), '-i', item);
    }
  }

  // Filter graph: scale+pad + optional zoompan, lalu xfade berurutan
  const filters = [];

  for (let i = 0; i < media.length; i++) {
    const isImg = isImage(media[i]);
    let chain = `[${i}:v]scale=${width}:${height}:force_original_aspect_ratio=decrease,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2,setsar=1`;

    if (kenBurns && isImg) {
      // Pick Ken Burns mode
      const mode = kenBurnsMode === 'random' ? KENBURNS_KEYS[i % KENBURNS_KEYS.length] : kenBurnsMode;
      const kbFilter = KENBURNS_MODES[mode] || KENBURNS_MODES['zoom-in'];
      chain += `,format=rgba,${kbFilter(width, height, fps, perImage)}`;
    }

    // Video items might have different fps — normalize
    if (!isImg) {
      chain += `,fps=${fps},setpts=PTS-STARTPTS`;
    }

    chain += `,format=yuv420p[img${i}]`;
    filters.push(chain);
  }

  if (media.length === 1) {
    // Single item with Ken Burns — no xfade needed
    filters.push(`[img0]null[vout]`);
  } else if (media.length === 2) {
    const offset = Math.max(0.1, perImage - transitionDuration);
    filters.push(
      `[img0][img1]xfade=transition=${transitionName}:duration=${transitionDuration}:offset=${offset}[vout]`,
    );
  } else {
    // N items: chain xfade sequentially
    let prev = 'img0';
    let accumulatedOffset = 0;

    for (let i = 1; i < media.length; i++) {
      const label = i === media.length - 1 ? 'vout' : `xf${i}`;
      accumulatedOffset += perImage - transitionDuration;
      filters.push(
        `[${prev}][img${i}]xfade=transition=${transitionName}:duration=${transitionDuration}:offset=${accumulatedOffset}[${label}]`,
      );
      prev = label;
    }
  }
...
async function buildPlaylistMixFile(files, config, workspaceDir, job) {
  ...
  const fmt = i =>
    `[${i}:a]aformat=sample_fmts=fltp:sample_rates=44100:channel_layouts=stereo${silence > 0 ? `,apad=pad_dur=${silence}` : ''}[a${i}]`;
...
function qualityPreset(config) {
  ...
  if (perfPreset)
    return { preset: perfPreset, crf: String(config.target?.crf ?? (q === 'fast' ? 26 : q === 'high' ? 18 : 22)) };
...
async function encoderArgs(config) {
  ...
  let encText = '';
  try {
    encText = await run(FFMPEG, ['-hide_banner', '-encoders']);
  } catch {
    encText = '';
  }
...
function bumperWindows(position, bumperDuration, duration) {
  ...
  if (position === 'Keduanya')
    return [`between(t,0,${Math.min(duration, d)})`, `between(t,${Math.max(0, duration - d)},${duration})`];
...
function parseNowPlayingText(config = {}, audioFile = '', job = {}) {
  ...
  if (!title && sp.nowPlayingAutoFromFile && audioFile)
    title = path
      .basename(audioFile)
      .replace(/\.[^.]+$/, '')
      .replace(/[_-]+/g, ' ')
  ...
  return String(sp.nowPlayingTemplate || '{title}')
    .replaceAll('{title}', title || 'Now Playing')
...
function audioProcessingChain(inputLabel, outputLabel, config, duration) {
  ...
  if (a.noiseGate)
    filters.push(`agate=threshold=${Number(a.noiseGateThreshold ?? -45)}dB:ratio=2:attack=20:release=250`);
  ...
  if (a.compressor)
    filters.push(
      `acompressor=threshold=${Number(a.compressorThreshold ?? -18)}dB:ratio=${Number(a.compressorRatio ?? 3)}:attack=20:release=250`,
    );
  ...
  try {
    const child = spawn(
      ...
      [
        ...
        file,
        '-t',
        String(Math.max(1, Math.min(300, seconds))),
        '-ac',
        '1',
        '-ar',
        '8000',
    ...
    const count = Math.max(30, Math.min(500, buckets));
    const per = Math.max(1, Math.floor(samples / count));
    const peaks = [];
    for (let i = 0; i < count; i++) {
      let peak = 0;
      for (let j = i * per; j < Math.min(samples, (i + 1) * per); j++)
        peak = Math.max(peak, Math.abs(buf.readInt16LE(j * 2)) / 32768);
      peaks.push(peak);
    }
    return peaks;
  } catch {
...
function beatEnableExpression(peaks, seconds = 60, sensitivity = 55) {
  ...
  peaks.forEach((v, i) => {
    ...
    if (v >= threshold && v >= prev && v >= next) {
      const t = (i / peaks.length) * seconds;
      parts.push(`between(t,${Math.max(0, t - 0.05).toFixed(2)},${(t + 0.14).toFixed(2)})`);
    }
  });
  return parts.slice(0, 160).join('+');
}

export function resolveLayerOrder(layerOrder) {
  const defaultOrder = [
    'bumper',
    'particle',
    'logo',
  ...
  const parsed = String(layerOrder || '')
    .split(',')
    .map(part => part.trim())
    .filter(Boolean);
  const supported = new Set(defaultOrder);
  const ordered = [];
  for (const key of parsed) {
    if (!supported.has(key) || ordered.includes(key)) continue;
    ordered.push(key);
  }
...
export async function buildFfmpegArgs(job, config, workspaceDir) {
  let visual = job.input?.visual || config.input.visual;
  let audio = job.input?.audio || config.input.audio;
  const galleryVisual = await buildGallerySlideshowFile(config, workspaceDir, job);
  if (galleryVisual) visual = galleryVisual;
  ...
  if (config.audio?.mixMode === 'playlist') {
    const playlistFiles = [
      ...(config.audio?.introSongs || []),
      ...(config.audio?.songs?.length ? config.audio.songs : [audio]),
      config.audio?.endingSong,
    ].filter(Boolean);
  ...
  for (const [key, file] of Object.entries({
    ...
    bumper,
    cta,
    overlayAsset: config.overlay?.overlayFile,
    ambient: config.audio?.ambientLoop,
    voice: config.audio?.voiceTrack,
    effect: config.audio?.effectTrack,
  })) {
    if (hasFile(file)) {
      if (
        key === 'particle' ||
  ...
  for (const stem of config.audio?.stems || []) {
    if (hasFile(stem?.file) && stem?.mute !== true) {
      indexes.stems.push({
        index: nextIndex++,
        volume: Number(stem.volume ?? 100),
        pan: Number(stem.pan ?? 0),
        name: stem.name || 'stem',
      });
  ...
  // Shared params for all per-layer ASS documents (title/timestamp/lowerThird/watermark/lyrics)
  const assBaseParams = {
    rows: lyricRows,
  ...
  const textLayerActive = {
    nowPlaying: Boolean(assBaseParams.titleEnabled && assBaseParams.title),
    timestamp: Boolean(assBaseParams.timestampEnabled),
    lowerThird: Boolean(assBaseParams.lowerThirdEnabled && assBaseParams.lowerThirdText),
    watermark: Boolean(assBaseParams.watermarkEnabled && assBaseParams.watermark),
    lyrics: lyricRows.length > 0,
  };
  async function writeLayerAss(layerKey) {
    const doc = buildAssDocument({ ...assBaseParams, include: layerKey });
    if (!doc) return '';
    const file = path.join(workspaceDir, 'tmp', `${job.id || Date.now()}-${layerKey}.ass`);
  ...
  const filters = [];
  filters.push(
    `[0:v]scale=${width}:${height}:force_original_aspect_ratio=decrease,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2,setsar=1,format=rgba[v0]`,
  );
  const introDelay =
    config.branding?.bumperEnabled &&
    (config.branding?.bumperPosition === 'Awal' || config.branding?.bumperPosition === 'Keduanya') &&
    config.branding?.bumperAudioMode === 'after-intro'
      ? Math.round(Number(config.branding?.bumperDuration || 0) * 1000)
      : 0;
  ...
  if (indexes.voice !== undefined) {
    filters.push(
      `[${indexes.voice}:a]volume=${Number(config.audio?.voiceVolume ?? 100) / 100},atrim=0:${duration}[voice]`,
    );
    mixLabels.push('voice');
  }
  if (indexes.effect !== undefined) {
    filters.push(
      `[${indexes.effect}:a]volume=${Number(config.audio?.effectVolume ?? 80) / 100},atrim=0:${duration}[effect]`,
    );
  ...
  if (mixLabels.length > 1)
    filters.push(
      `${mixLabels.map(x => `[${x}]`).join('')}amix=inputs=${mixLabels.length}:duration=first:dropout_transition=2[a_mixed]`,
    );
  ...
  const detectedBeatEnable =
    config.audio?.beatDetection !== false
      ? beatEnableExpression(
          await waveformPeaks(audio, Math.min(duration, 90), 240),
          Math.min(duration, 90),
          config.spectrum?.beatSensitivity ?? config.audio?.reactiveStrength ?? 55,
        )
      : '';
  let v = 'v0';
  let n = 1;

  // --- Reorderable video layers (branding.layerOrder controls bumper/particle/logo/cta/spectrum) ---
  function applyBumperLayer() {
    if (indexes.bumper === undefined || !config.branding?.bumperEnabled) return;
    ...
    windows.forEach((expr, i) => {
      filters.push(`[${v}][${labels[i]}]overlay=0:0:enable='${expr}':shortest=0[v${n}]`);
      v = `v${n++}`;
    });
  }

  function applyParticleLayer() {
    if (indexes.particle === undefined || config.overlay.videoParticle === false) return;
    const opacity = Number(config.overlay.particleOpacity ?? 70) / 100;
    filters.push(
      `[${indexes.particle}:v]scale=${width}:${height},format=rgba,colorchannelmixer=aa=${opacity}[particle]`,
    );
    filters.push(`[${v}][particle]overlay=0:0:shortest=1[v${n}]`);
    v = `v${n++}`;

    // overlayAsset and beat-reactive flash aren't user-orderable; keep them adjacent to particle as before
    if (indexes.overlayAsset !== undefined && config.overlay?.overlayEnabled) {
      const scale = Number(config.overlay.overlayScale || 100) / 100;
      const overlayOpacity = Number(config.overlay.overlayOpacity ?? 80) / 100;
      ...
      filters.push(
        `[${indexes.overlayAsset}:v]scale=iw*${scale}:-1,format=rgba,colorchannelmixer=aa=${overlayOpacity}[overlayAsset]`,
      );
      filters.push(`[${v}][overlayAsset]overlay=${xy}:enable='${enable}'[v${n}]`);
      v = `v${n++}`;
    }
    ...
    if (config.audio?.reactiveFx && config.audio.reactiveFx !== 'Off') {
      ...
      if (reactive) {
        ...
        filters.push(
          `[${v}]drawbox=x=0:y=0:w=iw:h=ih:color=${color}@${reactive.opacity}:t=fill:enable='${enable}'[v${n}]`,
        );
        v = `v${n++}`;
      }
    }
  ...
  function applyLogoLayer() {
    if (indexes.logo === undefined || config.branding?.logoEnabled === false) return;
    ...
    const [lx, ly] = overlayPositionAnimated(
      config.branding.logoPosition || 'Kanan Atas',
      Number(config.branding.logoMarginX || 20),
      Number(config.branding.logoMarginY || 20),
      config,
      config.branding.logoAnimation || 'none',
      startLogo,
      endLogo,
      duration,
    );
    ...
    const fadeChain =
      config.branding.logoAnimation === 'fade'
        ? `,fade=t=in:st=${startLogo}:d=${fade}:alpha=1,fade=t=out:st=${Math.max(startLogo, endLogo - fade)}:d=${fade}:alpha=1`
        : '';
    filters.push(
      `[${indexes.logo}:v]scale=iw*${scale}:-1,format=rgba,colorchannelmixer=aa=${opacity}${fadeChain}[logo]`,
    );
    filters.push(`[${v}][logo]overlay=x='${lx}':y='${ly}':enable='${enable}'[v${n}]`);
    v = `v${n++}`;
  }

  function applyCtaLayer() {
    if (indexes.cta === undefined || !config.branding?.ctaEnabled) return;
    ...
    const pos =
      preset === 'center-cta'
        ? 'Tengah'
        : preset === 'like-subscribe-bottom'
          ? 'Kanan Bawah'
          : config.branding.ctaPosition || 'Kanan Bawah';
    ...
    const chroma =
      config.branding.ctaChromaPreset === 'blue' ? '0x0000ff' : config.branding.ctaChromaColor || '0x00ff00';
    const similarity = Number(
      config.branding.ctaSimilarity ?? (config.branding.ctaChromaPreset === 'auto' ? 0.28 : 0.35),
    );
    const blend = Number(config.branding.ctaBlend ?? 0.08);
    filters.push(
      `[${indexes.cta}:v]scale=iw*${ctaScale}:-1,colorkey=${chroma}:${similarity}:${blend},format=rgba[cta]`,
    );
    filters.push(`[${v}][cta]overlay=${xy}:enable='between(t,${startAt},${endAt})'[v${n}]`);
    v = `v${n++}`;
  }

  function applySpectrumLayer() {
    if (config.spectrum?.enabled === false) return;
    const sp = config.spectrum || {};
    const colors = (sp.colors?.length ? sp.colors : [sp.color1 || 'white', sp.color2].filter(Boolean))
      .map(normalizeColor)
      .join('|');
    ...
    const specW =
      sp.widthMode === 'center'
        ? Math.round((width * Number(sp.zoom || 100)) / 100)
        : width - Number(sp.marginX || 0) * 2;
    ...
    if (sp.analyzerMode === 'frequency' || sp.model === 'Bar') {
      filters.push(
        `[awave]showfreqs=s=${specW}x${specH}:mode=bar:ascale=log:fscale=log:colors=${colors},format=rgba,colorchannelmixer=aa=${opacity}[waves0]`,
      );
    } else {
      filters.push(
        `[awave]showwaves=s=${specW}x${specH}:mode=${mode}:colors=${colors},format=rgba,colorchannelmixer=aa=${opacity}[waves0]`,
      );
    }
    ...
    filters.push(`[${v}][${waveLabel}]overlay=${xy}[v${n}]`);
    v = `v${n++}`;
    if (sp.glow) {
      const glowOpacity = Math.max(0.03, Math.min(0.35, Number(sp.glowStrength || 35) / 100));
      filters.push(
        `[${v}]drawbox=x=0:y=0:w=iw:h=ih:color=${normalizeColor(sp.color2 || sp.color1 || 'white')}@${glowOpacity}:t=fill:enable='${detectedBeatEnable || 'gt(sin(t*10),0.82)'}'[v${n}]`,
      );
      v = `v${n++}`;
    }
  }

  async function applyAssLayer(layerKey) {
    if (!textLayerActive[layerKey]) return;
    const assFile = await writeLayerAss(layerKey);
    if (!assFile) return;
    filters.push(`[${v}]ass='${escapeFilter(assFile)}'[v${n}]`);
    v = `v${n++}`;
  }

  const layerApply = {
    bumper: applyBumperLayer,
    particle: applyParticleLayer,
    logo: applyLogoLayer,
    cta: applyCtaLayer,
    spectrum: applySpectrumLayer,
    lyrics: async () => applyAssLayer('lyrics'),
    watermark: async () => applyAssLayer('watermark'),
    nowPlaying: async () => applyAssLayer('nowPlaying'),
    timestamp: async () => applyAssLayer('timestamp'),
    lowerThird: async () => applyAssLayer('lowerThird'),
  };
  for (const key of resolveLayerOrder(config.branding?.layerOrder)) {
    await layerApply[key]?.();
  }
  ...
  if (config.spectrum?.progressBar !== false) {
    ...
    filters.push(`[${v}]drawbox=x=80:y=ih-24:w=(iw-160)*t/${duration}:h=${ph}:color=${pc}@0.9:t=fill[v${n}]`);
    v = `v${n++}`;
  }

  if (config.overlay?.darken) {
    filters.push(
      `[${v}]drawbox=x=0:y=0:w=iw:h=ih:color=black@${Number(config.overlay.darkenOpacity || 15) / 100}:t=fill[v${n}]`,
    );
    v = `v${n++}`;
  }
  if (config.overlay?.vignette) {
    filters.push(`[${v}]vignette=angle=PI/4:mode=backward[v${n}]`);
    v = `v${n++}`;
  }
  if (config.overlay?.filmGrain) {
    filters.push(`[${v}]noise=alls=${Math.max(0, Number(config.overlay.grainStrength || 12))}:allf=t+u[v${n}]`);
    v = `v${n++}`;
  }
  if (config.overlay?.scanlines) {
    filters.push(
      `[${v}]drawgrid=width=iw:height=4:thickness=1:color=black@${Number(config.overlay.scanlineOpacity || 6) / 100}[v${n}]`,
    );
    v = `v${n++}`;
  }
  if (config.overlay?.letterbox) {
    ...
    filters.push(
      `[${v}]drawbox=x=0:y=0:w=iw:h=${lb}:color=black@1:t=fill,drawbox=x=0:y=ih-${lb}:w=iw:h=${lb}:color=black@1:t=fill[v${n}]`,
    );
    v = `v${n++}`;
  }
  if (config.overlay?.frameBorder) {
    const bc = normalizeColor(config.overlay.borderColor || 'white');
    const bt = Math.max(1, Number(config.overlay.borderThickness || 6));
    filters.push(`[${v}]drawbox=x=0:y=0:w=iw:h=ih:color=${bc}@0.95:t=${bt}[v${n}]`);
    v = `v${n++}`;
  }
  ...
  if (Number(config.performance?.ffmpegThreads || 0) > 0)
    args.push('-threads', String(Math.max(1, Math.min(32, Number(config.performance.ffmpegThreads)))));
  args.push(
    '-b:v',
    config.target.bitrate || '4500k',
    '-pix_fmt',
    config.target.pixelFormat || 'yuv420p',
    '-c:a',
    'aac',
    '-b:a',
    config.audio.audioBitrate || '192k',
    '-shortest',
  );
  ...
  return new Promise((resolve, reject) => {
      ...
      for (const line of buf.toString().split(/\r?\n/)) {
        ...
        if (key === 'out_time_ms') {
          ...
          if (Number.isFinite(micros) && duration > 0)
            callbacks.onProgress?.(Math.min(99, Math.max(1, Math.round((micros / 1_000_000 / duration) * 100))));
        }
    ...
    child.on('close', code => {
      if (code === 0) {
        ...
        callbacks.onLog?.(`Render selesai: ${output}`);
