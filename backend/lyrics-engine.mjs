...
export function rowsToLrc(rows) {
  return (
    rows
      .map(
        r =>
          `[${String(Math.floor(Number(r.time || 0) / 60)).padStart(2, '0')}:${String(Math.floor(Number(r.time || 0) % 60)).padStart(2, '0')}.${String(Math.round((Number(r.time || 0) % 1) * 100)).padStart(2, '0')}] ${r.text}`,
      )
      .join('\n') + '\n'
  );
}
...
export function scoreLyricTimeline(rows, audioDuration, config = {}, beats = []) {
  ...
  rows.forEach((r, i) => {
    ...
    if (dur < minDur) {
      score -= 5;
      warnings.push(`baris ${i + 1}: durasi terlalu cepat`);
    }
    if (dur > maxDur && rows.length > 1) {
      score -= 2;
      warnings.push(`baris ${i + 1}: durasi terlalu lama`);
    }
    if (cps > cpsTarget * 1.35) {
      score -= 4;
      warnings.push(`baris ${i + 1}: reading speed terlalu cepat`);
    }
    if (i > 0 && Number(r.time) <= Number(rows[i - 1].time)) {
      score -= 12;
      warnings.push(`baris ${i + 1}: timestamp tidak naik`);
    }
  });
  const last = rows.at(-1)?.time || 0;
  if (audioDuration && last > audioDuration + 2) {
    score -= 10;
    warnings.push('timestamp melebihi durasi audio');
  }
  if (config.lyrics?.beatSnap && beats?.length) {
    const snapped = rows.filter(r =>
      beats.some(b => Math.abs(Number(b.time ?? b) - Number(r.time)) <= Number(config.lyrics?.beatSnapWindow || 0.22)),
    ).length;
    if (snapped / rows.length < 0.35) {
      score -= 3;
      warnings.push('sedikit lirik yang snap ke beat');
    }
  }
  return {
    score: Math.max(0, Math.min(100, Math.round(score))),
    warnings: [...new Set(warnings)].slice(0, 30),
    metrics: { lines: rows.length, audioDuration, beats: beats?.length || 0 },
  };
}
...
export function autoAlignLyrics(text, audioDuration, config = {}, beats = []) {
  ...
  if (!useSmart || fixed > 0) {
    ...
    for (let i = 0; i < lines.length; i++) {
      let start = cursor;
      if (config.lyrics?.beatSnap !== false)
        start = snapToNearestBeat(start, beats, Number(config.lyrics?.beatSnapWindow || 0.22));
...
export async function saveLyricsExport(rows, format, outFile) {
  ...
  const content = fmt === 'lrc' ? rowsToLrc(rows) : fmt === 'vtt' ? rowsToVtt(rows) : lrcRowsToSrt(rows);
  if (outFile) {
