...
export function outputName(title, config, index = 1) {
  const safeTitle = String(title || `render-${index}`)
    .replace(/[\\/:*?"<>|]/g, '-')
    .replace(/\s+/g, ' ')
    .trim();
  const d = new Date();
  ...
  return (
    String(config.target?.outputPattern || '{title}-{date}-{num}')
      .replaceAll('{title}', safeTitle)
      .replaceAll('{date}', date)
      .replaceAll('{time}', time)
      .replaceAll('{num}', num) + '.mp4'
  );
}
...
export function ensureUniqueOutput(fullPath, overwrite) {
  ...
  const extn = path.extname(fullPath);
  const stem = path.basename(fullPath, extn);
  let i = 2;
  let candidate = path.join(dir, `${stem}-${i}${extn}`);
...
export function validateRenderConfig(config = {}, options = {}) {
  ...
  const galleryImages = (gallery?.enabled ? gallery.images || [] : []).filter(
    f => f && existsSync(f) && (isImage(f) || isVideo(f)),
  );
  const galleryActive = Boolean(gallery?.enabled && galleryImages.length > 0);

  if (galleryActive) {
    if (galleryImages.length < (gallery.images || []).filter(Boolean).length)
      warnings.push('Beberapa file gallery tidak ditemukan atau format tidak didukung; hanya file valid yang dipakai.');
  } else if (!visual) errors.push('File visual belum dipilih.');
  else if (!existsSync(visual)) errors.push(`File visual tidak ditemukan: ${visual}`);
  else if (!isVideo(visual) && !isImage(visual))
    errors.push(`Format visual belum didukung: ${path.extname(visual) || visual}`);
  ...
  if (outputDir) {
    ...
    if (!existsSync(dir) && !existsSync(parent))
      warnings.push(`Folder output belum ada dan parent tidak ditemukan: ${dir}`);
  }
  if (config.spectrum?.enabled !== false && !audio) warnings.push('Spectrum aktif tetapi audio belum siap.');
  if (config.branding?.logoEnabled && config.branding?.logo && !existsSync(config.branding.logo))
    warnings.push('Logo aktif tetapi file logo tidak ditemukan.');
  if (config.overlay?.overlayEnabled && config.overlay?.overlayFile && !existsSync(config.overlay.overlayFile))
    warnings.push('Overlay aktif tetapi file overlay tidak ditemukan.');
  if (config.overlay?.videoParticle && config.overlay?.particleFile && !existsSync(config.overlay.particleFile))
    warnings.push('Particle aktif tetapi file particle tidak ditemukan.');
  ...
  if (
    ...
    config.spectrum?.enabled !== false &&
    lyricPos === 'Bawah' &&
    spectrumPos === 'Bawah'
  )
    warnings.push('Lirik dan spectrum sama-sama di bawah; pertimbangkan naikkan salah satunya.');
  if (
    config.overlay?.lowerThirdEnabled &&
    config.lyrics?.enabled !== false &&
    lowerThirdPos === 'Bawah' &&
    lyricPos === 'Bawah'
  )
    warnings.push('Lower third dan lirik sama-sama di bawah; ada risiko tabrakan.');
  if (
    config.overlay?.lowerThirdEnabled &&
    config.spectrum?.enabled !== false &&
    lowerThirdPos === 'Bawah' &&
    ...
    warnings.push('Lower third dan spectrum sama-sama di bawah; hasil bisa terlalu padat.');
  if (
    config.overlay?.timestamp &&
    config.spectrum?.nowPlaying &&
    timestampPos.includes('Atas') &&
    String(config.spectrum?.nowPlayingPosition || 'Atas') === 'Atas'
  )
    warnings.push('Timestamp dan now playing sama-sama di area atas; cek jarak aman.');
  if (
    height > width &&
    config.overlay?.lowerThirdEnabled &&
    lowerThirdPos === 'Bawah' &&
    Number(config.overlay?.lowerThirdDuration || 0) > 6
  )
    warnings.push('Lower third cukup lama untuk format vertical; pertimbangkan durasi lebih pendek.');
...
export function targetRiskSummary(config, files = [], pairs = [], collisions = []) {
  ...
  const score = Math.max(
    0,
    100 - brokenFiles * 25 - invalidFiles * 10 - lowConfidencePairs * 8 - fallbackPairs * 6 - collisionCount * 5,
  );
  return {
    score,
    level: score >= 85 ? 'Ready' : score >= 65 ? 'Check' : 'Risk',
    invalidFiles,
    brokenFiles,
    lowConfidencePairs,
    fallbackPairs,
...
export function autoTargetTune(config, files = [], pairs = []) {
  ...
  const avgDuration = Math.round(
    (pairs
      .map(p => files.find(f => f.path === p.audio)?.duration)
      .filter(Boolean)
      .reduce((a, b) => a + b, 0) ||
      Number(config.target?.duration || 0) * jobCount ||
      180) / Math.max(1, pairs.map(p => files.find(f => f.path === p.audio)?.duration).filter(Boolean).length || 1),
  );
  const target = portrait
    ? { resolution: '1080x1920', width: 1080, height: 1920, aspect: '9:16', bitrate: jobCount > 20 ? '5500k' : '7000k' }
    : square
      ? { resolution: '1080x1080', width: 1080, height: 1080, aspect: '1:1', bitrate: '6000k' }
      : {
          resolution: width >= 1600 ? '1920x1080' : '1280x720',
          width: width >= 1600 ? 1920 : 1280,
          height: width >= 1600 ? 1080 : 720,
          aspect: '16:9',
          bitrate: width >= 1600 ? '8000k' : '4500k',
        };
  ...
  const reasons = [
    `visual ${portrait ? 'portrait' : square ? 'square' : 'landscape'}`,
    `${jobCount} job`,
    `durasi rata-rata ${avgDuration || target.duration}s`,
  ];
...
export function pairMedia(files, config = {}) {
  ...
  const lyrics = files.filter(f => f.type === 'lyrics');
  const mode = config.input?.pairMode || 'by-name';
  const ignore = config.input?.ignoreWords || '';
  const fallbackVisual = config.input?.visual || visuals[0]?.path || '';
  ...
  if (mode === 'one-audio-all-visual') {
    ...
    return visuals
      .map((visual, i) => {
        const title = path.basename(visual.name).replace(/\.[^.]+$/, '');
        return {
          title,
          visual: visual.path,
          audio,
          lyrics: '',
          index: i + 1,
          outputName: outputName(title, config, i + 1),
          confidence: 80,
          pairReason: 'one audio untuk semua visual',
          lyricConfidence: 0,
        };
      })
      .filter(x => x.audio);
  ...
  return audios.map((audio, i) => {
    const aBase = normalizeName(audio.name, ignore);
    let visual = fallbackVisual;
    let lyric = '';
    let confidence = 45;
    let pairReason = 'fallback visual';
    let lyricConfidence = 0;
    let lyricReason = 'lirik kosong';
    if (mode === 'by-order') {
      visual = pickByOrder(visuals, i) || fallbackVisual;
      confidence = visual ? 75 : 0;
      pairReason = 'by-order';
    } else if (mode === 'random-visual') {
      visual =
        pickByOrder(
          [...visuals].sort(() => Math.random() - 0.5),
          i,
        ) || fallbackVisual;
      confidence = visual ? 65 : 0;
      pairReason = 'random visual';
    } else {
      const exact = visuals.find(v => normalizeName(v.name, ignore) === aBase);
      const fuzzy = visuals
        .map(v => ({ v, score: similarity(normalizeName(v.name, ignore), aBase) }))
        .sort((a, b) => b.score - a.score)[0];
      if (exact) {
        visual = exact.path;
        confidence = 100;
        pairReason = 'exact name match';
      } else if (fuzzy?.score >= 0.34) {
        visual = fuzzy.v.path;
        confidence = Math.round(fuzzy.score * 100);
        pairReason = `fuzzy match ${(fuzzy.score * 100).toFixed(0)}%`;
      }
      const lExact = lyrics.find(l => normalizeName(l.name, ignore) === aBase);
      const lFuzzy = lyrics
        .map(l => ({ l, score: similarity(normalizeName(l.name, ignore), aBase) }))
        .sort((a, b) => b.score - a.score)[0];
      if (lExact) {
        lyric = lExact.path;
        lyricConfidence = 100;
        lyricReason = 'exact lirik';
      } else if (lFuzzy?.score >= 0.34) {
        lyric = lFuzzy.l.path;
        lyricConfidence = Math.round(lFuzzy.score * 100);
        lyricReason = `fuzzy lirik ${(lFuzzy.score * 100).toFixed(0)}%`;
      }
    }
    if (!visual) {
      confidence = 0;
      pairReason = 'visual tidak ditemukan';
    }
...
export function estimateRender(config, pairs = [], files = []) {
  ...
  const duration =
    Number(config.target?.duration || 0) ||
    Math.round(durations.reduce((a, b) => a + b, 0) / Math.max(1, durations.length)) ||
    180;
  ...
  return {
    jobs,
    durationPerJob: duration,
    estimatedTotalDuration: duration * jobs,
    estimatedSizeMB: sizeMB,
    bitrate: `${bitrateNum}k`,
    audioBitrate: `${audioBitrate}k`,
    resolution: config.target?.resolution || `${config.target?.width || 1280}x${config.target?.height || 720}`,
    encoder: config.target?.hardwareAccel || 'auto',
  };
}
...
export function targetSummary(config, files = [], pairs = []) {
  ...
  const lyrics = files.filter(f => f.type === 'lyrics').length;
  const warnings = files.flatMap(f => (f.warnings || []).map(w => `${f.name}: ${w}`));
  const errors = [];
  if (!config.input?.visual && videos + images === 0)
    errors.push('Visual belum dipilih dan folder tidak berisi video/gambar.');
  ...
  return {
    counts: { videos, images, audios, lyrics, total: files.length },
    resolution,
    ready: errors.length === 0 && risk.level !== 'Risk',
    errors,
    warnings: [...warnings, ...(risk.warnings || [])].slice(0, 80),
    estimate: estimateRender(config, pairs, files),
    collisions,
    risk,
    autoTune,
  };
